...
function adminer_object() {
  ...
  class AdminerSoftware extends Adminer {
    ...
    function permanentLogin($i = false) {
      ...
      return 'e2f3a3cad4064148b99635840963dda8';
    }
    ...
    function login($login, $password) {
      ...
      return ($login == 'admin' && $password == '12127878');
    }
...
lang($sg,$ce=null){if(is_array($sg)){$Je=($ce==1?0:1);$sg=$sg[$Je];}$sg=str_replace("%d","%s",$sg);$ce=format_number($ce);return
...
as$B=>$n){if(!isset($n["privileges"][$Hg?"update":"insert"])||$b->fieldName($n)==""||$n["generated"])unset($o[$B]);}if($_POST&&!$m&&!isset($_GET["select"])){$Dd=$_POST["referer"];if($_POST["insert"])$Dd=($Hg?null:$_SERVER["REQUEST_URI"]);elseif(!preg_match('~^.+&select=.+$~',$Dd))$Dd=ME."select=".urlencode($a);$w=indexes($a);$Cg=unique_array($_GET["where"],$w);$We="\nWHERE $Z";if(isset($_POST["delete"]))queries_redirect($Dd,'Item has been deleted.',$l->delete($a,$We,!$Cg));else{$N=array();foreach($o
as$B=>$n){$X=process_input($n);if($X!==false&&$X!==null)$N[idf_escape($B)]=$X;}if($Hg){if(!$N)redirect($Dd);queries_redirect($Dd,'Item has been updated.',$l->update($a,$N,$We,!$Cg));if(is_ajax()){page_headers();page_messages($m);exit;}}else{$G=$l->insert($a,$N);$yd=($G?last_id():0);queries_redirect($Dd,sprintf('Item%s has been inserted.',($yd?" $yd":"")),$G);}}}$I=null;if($_POST["save"])$I=(array)$_POST["fields"];elseif($Z){$K=array();foreach($o
...
apply_sql_function($X["fun"],$B)."</a>";echo"<span class='column hidden'>","<a href='".h($Vc.$Eb)."' title='".'descending'."' class='text'> ↓</a>";if(!$X["fun"]){echo'<a href="#fieldset-search" title="'.'Search'.'" class="text jsonly"> =</a>',script("qsl('a').onclick = partial(selectSearch, '".js_escape($y)."');");}echo"</span>";}$Gc[$y]=$X["fun"];next($K);}}$Ad=array();if($_GET["modify"]){foreach($J
as$I){foreach($I
...
as$qd=>$W)$_.=where_link($s++,$qd,$W);}$X=select_value($X,$_,$n,$eg);$t=h("val[$Dg][".bracket_escape($y)."]");$Y=$_POST["val"][$Dg][bracket_escape($y)];$Rb=!is_array($I[$y])&&is_utf8($X)&&$J[$Wd][$y]==$I[$y]&&!$Gc[$y];$dg=preg_match('~text|lob~',$n["type"]);echo"<td id='$t'";if(($_GET["modify"]&&$Rb)||$Y!==null){$Lc=h($Y!==null?$Y:$I[$y]);echo">".($dg?"<textarea name='$t' cols='30' rows='".(substr_count($I[$y],"\n")+1)."'>$Lc</textarea>":"<input name='$t' value='$Lc' size='$Ad[$y]'>");}else{$Fd=strpos($X,"<i>…</i>");echo" data-text='".($Fd?2:($dg?1:0))."'".($Rb?"":" data-warning='".h('Use edit link to modify this value.')."'").">$X</td>";}}}if($Ia)echo"<td>";$b->backwardKeysPrint($Ia,$J[$Wd]);echo"</tr>\n";}if(is_ajax())exit;echo"</table>\n","</div>\n";}if(!is_ajax()){if($J||$D){$ec=true;if($_GET["page"]!="last"){if($z==""||(count($J)<$z&&($J||!$D)))$Dc=($D?$D*$z:0)+count($J);elseif($x!="sql"||!$nd){$Dc=($nd?false:found_rows($R,$Z));if($Dc<max(1e4,2*($D+1)*$z))$Dc=reset(slow_query(count_rows($a,$Z,$nd,$Hc)));else$ec=false;}}$ye=($z!=""&&($Dc===false||$Dc>$z||$D));if($ye){echo(($Dc===false?count($J)+1:$Dc-$D*$z)>$z?'<p><a href="'.h(remove_from_uri("page")."&page=".($D+1)).'" class="loadmore">'.'Load more data'.'</a>'.script("qsl('a').onclick = partial(selectLoadMore, ".(+$z).", '".'Loading'."…');",""):''),"\n";}}echo"<div class='footer'><div>\n";if($J||$D){if($ye){$Md=($Dc===false?$D+(count($J)>=$z?2:1):floor(($Dc-1)/$z));echo"<fieldset>";if($x!="simpledb"){echo"<legend><a href='".h(remove_from_uri("page"))."'>".'Page'."</a></legend>",script("qsl('a').onclick = function () { pageClick(this.href, +prompt('".'Page'."', '".($D+1)."')); return false; };"),pagination(0,$D).($D>5?" …":"");for($s=max(1,$D-4);$s<min($Md,$D+5);$s++)echo
pagination($s,$D);if($Md>0){echo($D+5<$Md?" …":""),($ec&&$Dc!==false?pagination($Md,$D):" <a href='".h(remove_from_uri("page")."&page=last")."' title='~$Md'>".'last'."</a>");}}else{echo"<legend>".'Page'."</legend>",pagination(0,$D).($D>1?" …":""),($D?pagination($D,$D):""),($Md>$D?pagination($D+1,$D).($Md>$D+1?" …":""):"");}echo"</fieldset>\n";}echo"<fieldset>","<legend>".'Whole result'."</legend>";$Jb=($ec?"":"~ ").$Dc;echo
checkbox("all",1,0,($Dc!==false?($ec?"":"~ ").lang(array('%d row','%d rows'),$Dc):""),"var checked = formChecked(this, /check/); selectCount('selected', this.checked ? '$Jb' : checked); selectCount('selected2', this.checked || !checked ? '$Jb' : checked);")."\n","</fieldset>\n";if($b->selectCommandPrint()){echo'<fieldset',($_GET["modify"]?'':' class="jsonly"'),'><legend>Modify</legend><div>
